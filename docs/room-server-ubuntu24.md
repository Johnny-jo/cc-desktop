...
- 托管群此版限定为文字聊天、引用/撤回与成员管理；不承诺附件、Agent 或 Mod 在服务器运行。局域网原有功能不受此限制。
...
本次验证记录（Windows / Node 22.23.2）：类型检查与桌面生产构建通过；托管服务、恢复及多 Agent 回归 39 项通过（测试超时配置为 15 秒）；shared 228 项通过。扩大回归中附件缓存/路径相关的 12 项仍失败，未改动对应缓存实现，不能声称全部测试通过。多 Agent 回归曾在默认 5 秒限制下超时，增加测试运行预算后通过。
